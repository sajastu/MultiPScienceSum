...
class TransformerEncoderLayer(nn.Module):
    def __init__(self, d_model, heads, d_ff, dropout, topic_dim=100):
    ...
    def forward(self, iter, inputs, topic_input, mask):
        if (iter != 0):
        ...
        context, _ = self.self_attn(input_norm, input_norm, input_norm, topic_vec=topic_norm,
                                    mask=mask, type='self')
...
class TransformerEncoder(nn.Module):
    def __init__(self, d_model, d_ff, heads, dropout, num_inter_layers=0, topic_dim=100):
        super(TransformerEncoder, self).__init__()
        self.num_inter_layers = num_inter_layers
        self.pos_emb = PositionalEncoding(dropout, d_model)
        self.transformer = nn.ModuleList(
            [TransformerEncoderLayer(d_model, heads, d_ff, dropout, topic_dim)
             for _ in range(num_inter_layers)])
        self.layer_norm = nn.LayerNorm(d_model, eps=1e-6)

    def forward(self, hidden_states, topic_emb, attention_mask=None):
        """ See :obj:`EncoderBase.forward()`"""

        x = self.pos_emb(hidden_states)

        for i in range(self.num_inter_layers):
            x = self.transformer[i](i, x, topic_emb, attention_mask)  # all_sents * max_tokens * dim
